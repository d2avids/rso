...
from competitions.models import Q11, Q11Report


@pytest.mark.django_db(transaction=True, reset_sequences=True)
class TestQ11ViewSet:
    competition_url = '/api/v1/competitions/'
    question_url = '/reports/q11/'
    ...
    def test_get_list_reg_commissar(
            ...
            competition, report_question11_not_verif, report_question11_verif2,
            report_question11_not_verif3
    ):
        """
    ...
    def test_get_list_not_auth(
            self, client, competition, report_question11_not_verif,
    ):
        """
    ...
    def test_create_report_participant(
        ...
        competition, detachment_competition, report_question11_not_verif2
    ):
        """
        ...
        global_report = Q11Report.objects.filter(
    ...
    def test_create_report_participant_with_incorrect_prize_place(
        ...
        """
        Проверка, что пользователь - участник конкурса, не может создать отчет,
        если указано некорректное призовое место (11).
        """
        response = authenticated_client.post(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}', self.report_with_prize_place_11,
            format='json'
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Response code is not 400'
        ...
        assert response.data == {
            'prize_place': [
                'Значения 11 нет среди допустимых вариантов.'
            ]
        }
    ...
    def test_get_report_id_not_auth(
        ...
        response = client.get(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}{report_question11_not_verif.id}/'
        )
        assert response.status_code == HTTPStatus.UNAUTHORIZED, (
            'Response code is not 401'
        )
    ...
    def test_get_report_id_auth(
        self, authenticated_client, competition, report_question11_not_verif
    ):
        """
        Проверка, что авторизованный пользователь не может
        получить отчет.
    ...
    def test_get_report_id_participant(
        self, authenticated_client, participants_competition_tandem,
        competition, report_question11_not_verif, report_question11_not_verif2
    ):
        """
        ...
        response = authenticated_client.get(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}{report_question11_not_verif2.id}/'
        )
        assert response.status_code == HTTPStatus.FORBIDDEN, (
            'Response code is not 403'
        )
    ...
    def test_get_report_id_author(
        ...
        assert report['event_name'] == (
            report_question11_not_verif.event_name
        ), 'Incorrect event_name'
        assert report['prize_place'] == (
            report_question11_not_verif.prize_place
    ...
    def test_put_verif_report_participant(
        self, authenticated_client_3, participants_competition_tandem,
        competition, report_question11_verif, report_question11_not_verif2
    ):
        """
        ...
        response = authenticated_client_3.put(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}{report_question11_verif.id}/',
            data=self.report, format='json'
        )
        assert response.status_code == HTTPStatus.FORBIDDEN, (
            'Response code is not 403'
    ...
    def test_put_report_not_auth(
        ...
        competition, report_question11_not_verif
    ):
        """
        Проверка, что неавторизованный пользователь не может
        изменить отчет.
    ...
    def test_put_report_reg_commissar(
        ...
        competition, report_question11_verif, report_question11_verif2
    ):
        """
    ...
    def test_delete_report(
        ...
        report_question11_not_verif, report_question11_verif2,
        request
    ):
        """
    ...
    def test_accept_report(
        ...
        response = test_client.post(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}{report_question11_not_verif.id}/accept/'
        )
        assert response.status_code == expected_status, (
            'Response code is not ' + str(expected_status)
        )
        if response.status_code == HTTPStatus.OK:
            verif_report = Q11.objects.get(
                id=report_question11_not_verif.id
            )
            assert verif_report.is_verified is True, 'Отчет не подтвержден'
            scores = Q11Report.objects.all()
            assert (
                scores[0].score ==
    ...
    def test_put_number_participants_reg_commander(
            self, authenticated_client_commissar_regional_headquarter,
            participants_competition_tandem, competition,
            report_question11_verif, report_question11_verif_second
    ):
        """
        ...
        response = authenticated_client_commissar_regional_headquarter.patch(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}{report_question11_verif_second.id}/',
            data=self.report,
            format='json'
        )
        assert response.status_code == HTTPStatus.OK, (
        ...
        scores = Q11Report.objects.filter(
            competition=competition,
            detachment=report_question11_verif.detachment_report.detachment
        ).all()
        ...
        assert (
            scores[0].score ==
            (self.report['prize_place'] +
             report_question11_verif.prize_place) / 2
        ), (
    ...
    def test_me_with_report(
        self, authenticated_client_3, participants_competition_tandem,
        competition, report_question11_not_verif, report_question11_verif_second,
        report_question11_verif2
    ):
        """
        ...
        assert data[0]['id'] == report_question11_not_verif.id, (
            'Отчет не соответствует ожидаемому'
        )
        assert data[1]['id'] == report_question11_verif_second.id, (
            'Отчет не соответствует ожидаемому'
        )

    def test_me_without_report(
        self, authenticated_client_3, participants_competition_tandem,
        competition, report_question11_verif2
    ):
        """
    ...
    def test_post_double_report(
        self, authenticated_client_3, participants_competition_tandem,
        competition, report_question11_verif
    ):
        """
        ...
        response = authenticated_client_3.post(
            ...
            f'{self.question_url}', data=[{
                "event_name": report_question11_verif.event_name,
                "prize_place": report_question11_verif.prize_place
            }],
            format='json'
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
