...
from competitions.models import Q12, Q12Report


@pytest.mark.django_db(transaction=True, reset_sequences=True)
class TestQ12ViewSet:
    competition_url = '/api/v1/competitions/'
    question_url = '/reports/q12/'
    ...
    def test_create_report_participant(
        ...
        global_report = Q12Report.objects.filter(
    ...
    def test_create_report_participant_with_incorrect_prize_place(
        ...
        """
        Проверка, что пользователь - участник конкурса, не может создать отчет,
        если указано некорректное призовое место (12).
        """
        response = authenticated_client.post(
            f'{self.competition_url}{competition.id}'
            f'{self.question_url}', self.report_with_prize_place_12,
            format='json'
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Response code is not 400'
        ...
        assert response.data == {
            'prize_place': [
                'Значения 12 нет среди допустимых вариантов.'
            ]
        }
    ...
    def test_put_number_participants_reg_commander(
        ...
        scores = Q12Report.objects.filter(
            competition=competition,
            detachment=report_question12_verif.detachment_report.detachment
        ).all()
    ...
    def test_me_with_report(
        self, authenticated_client_3, participants_competition_tandem,
        competition, report_question12_not_verif, report_question12_verif_second,
